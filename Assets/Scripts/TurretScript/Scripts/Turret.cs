...
public partial class Turret : MonoBehaviour{
    ...
    public virtual void Update(){
        ...
        if(this.yawSegment && (this.yawLimit != 0f)){
            ...
            //targetRelative = this.yawSegment.TransformPoint(this.target);
            ...
            else this.yawSegment.rotation = Quaternion.Inverse(targetRotation);//this.yawSegment.rotation = targetRotation;
        }
        if(this.pitchSegment && (this.pitchLimit != 0f)){
            targetRelative = this.pitchSegment.InverseTransformPoint(this.target);
            //targetRelative = this.pitchSegment.TransformPoint(this.target);
            ...
            else this.pitchSegment.rotation = Quaternion.Inverse(targetRotation);  // targetRotation;
        }
